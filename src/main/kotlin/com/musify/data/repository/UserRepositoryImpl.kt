...
class UserRepositoryImpl : UserRepository {
    ...
    override suspend fun create(user: User, passwordHash: String): Result<User> = try {
        val newUser = dbQuery {
            val id = Users.insertAndGetId {
                ...
                it[phoneVerified] = user.phoneVerified
                it[createdAt] = LocalDateTime.now()
                it[updatedAt] = LocalDateTime.now()
            }

            Users.select { Users.id eq id }
                .map { it.toUserWithPhone() }
                .single()
        }
    ...
    override suspend fun markPhoneVerified(userId: Int) {
        dbQuery {
            Users.update({ Users.id eq userId }) {
                it[phoneVerified] = true
                it[smsVerificationCode] = null
                it[smsCodeExpiry] = null
                it[updatedAt] = LocalDateTime.now()
            }
        }
    }

    override suspend fun updateSMSCode(userId: Int, code: String, expiry: LocalDateTime) {
        dbQuery {
            Users.update({ Users.id eq userId }) {
                it[smsVerificationCode] = code
                it[smsCodeExpiry] = expiry
                it[updatedAt] = LocalDateTime.now()
            }
        }
    }
    ...
    private fun ResultRow.toUserWithPhone(): User {
        return User(
            id = this[Users.id].value,
            email = this[Users.email],
            ...
            twoFactorEnabled = this[Users.twoFactorEnabled],
            smsVerificationCode = this[Users.smsVerificationCode],
            smsCodeExpiry = this[Users.smsCodeExpiry],
            createdAt = this[Users.createdAt],
            updatedAt = this[Users.updatedAt]
        )
    }
